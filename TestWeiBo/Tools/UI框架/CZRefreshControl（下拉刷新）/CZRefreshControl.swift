...
/// 刷新状态切换的临界点
private let CZRefreshOffset: CGFloat = 126 //刷新控件的高度，美团为126,默认的是60
...
class CZRefreshControl: UIControl {
    ...
    */
    ...
    func beginRefreshing() {
        ...
        // 调整表格的间距
        /*
          松开手之后，offsetd触发监听回调，inset增加 CZRefreshOffset，此时offset回退到 inset.top
         的位置，此时self 的height就是0，但是 RefreshVIew是从xib加载的，高度不变，为了让RefreshVIew显示出来，clipToBounds=false即可，记住这个技巧
         */
    ...
    func endRefreshing() {
        ...
        // 判断状态，是否正在刷新，如果不是，直接返回 防止调用2次这个方法，增加间距
        if refreshView.refreshState != .WillRefresh {
            return
        }
